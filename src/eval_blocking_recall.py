...
# Subsample to speed up evaluation (e.g. 50k random S1 entities)
sampled_gt = gt_df.sample(n=50000, random_state=42)
...
print(f"Total positive pairs to find in sample: {len(gt_pairs)}")

# 2. Build index from S1
print("Building S1 index for sample...")
...
index_NoSpace = defaultdict(list) # For domain names like "MoyaWinvest.com"
index_Num = defaultdict(list) # For cross-lingual names with matching address numbers
...
for idx in range(n_s1):
    ...
    if cn:
        ...
        # Space-less name for domain match
    ...
    if ca:
        ...
        for num in nums:
            if len(num) >= 2: # Ignore single digit numbers to reduce collisions
                index_Num[(c, num)].append(idx)

# Remove overly common keys
...
def scan_file(path):
    ...
    while True:
        ...
        for eid, c, cn, ca in zip(eids, countries, cnames, caddrs):
            def _add(idx):
                s1_eid = s1_eids[idx]
...
recall = len(found_pairs) / len(gt_pairs) if gt_pairs else 0
print(f"\nBlocking Recall: {len(found_pairs)} / {len(gt_pairs)} ({recall*100:.2f}%)")
