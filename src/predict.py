...
Peak memory < 2 GB, runtime ~2 minutes on 11.7M records.
...
from .normalize import core_name, core_address, normalize_country, LEGAL_SUFFIXES, _ADDRESS_ABBREVIATIONS
...
def run_prediction(test_dir="dataset/test", output_dir="output", max_cands_per_s1=30):
    t_start = time.time()
    ...
    n_s1 = len(s1_df)
    print(f"Loaded {n_s1} Source 1 entities in {time.time() - t_start:.2f}s.")
    ...
    print("Building multi-channel inverted index...")
    t_idx = time.time()
    ...
    print(f"Index built in {time.time() - t_idx:.2f}s! Distinct N: {len(index_N)}, A: {len(index_A)}, NA: {len(index_NA)}")

    # Containers for results per S1 entity
    candidates_list = [set() for _ in range(n_s1)]
    matches_list = [set() for _ in range(n_s1)]

    # Function to scan another source
    def process_source(src_path, src_label):
        t_src = time.time()
        print(f"Processing {src_label} from {src_path}...")
        df = pl.read_csv(src_path, separator="\t")
        src_eids = df["entity_id"].to_list()
        src_countries = [normalize_country(c) for c in df["country"].to_list()]
        src_cnames = [_clean_name(x) for x in df["business_name"].to_list()]
        src_caddrs = [_clean_addr(x) for x in df["business_address"].to_list()]
        print(f"Read {len(df)} rows of {src_label} in {time.time() - t_src:.2f}s. Matching...")

        t_scan = time.time()
        for eid, c, cn, ca in zip(src_eids, src_countries, src_cnames, src_caddrs):
            # 1. Exact Name Channel (very high precision)
            ...
            # 2. Exact Address Channel (handles transliteration / DBA names)
            ...
            if nums and cn:
                ...
                if k in index_NA:
                    for idx in index_NA[k]:
                        if len(candidates_list[idx]) < max_cands_per_s1:
                            candidates_list[idx].add(eid)
                        # High precision filter: require name similarity >= 70
                ...
                if len(words) >= 2:
                    ...
                    if k in index_N2:
                        for idx in index_N2[k]:
                            if len(candidates_list[idx]) < max_cands_per_s1:
                                candidates_list[idx].add(eid)
                            # Conservative match condition
                            s1_ca = clean_addrs[idx]
                            if fuzz.ratio(clean_names[idx], cn) >= 85 or (ca and s1_ca and fuzz.ratio(s1_ca, ca) >= 75):
                                matches_list[idx].add(eid)

        print(f"Finished {src_label} matching in {time.time() - t_scan:.2f}s.")

    # Process Source 2 and Source 3
    process_source(s2_path, "Source 2")
    process_source(s3_path, "Source 3")
    ...
    total_cands = sum(len(c) for c in candidates_list)
    print("\n" + "=" * 50)
    print(f"Inference complete in {time.time() - t_start:.2f} seconds!")
    print(f"Total Source 1 entities: {n_s1}")
    print(f"Entities with >= 1 match: {n_with_match} ({n_with_match / n_s1 * 100:.1f}%)")
    print(f"Singletons (0 matches): {n_singletons} ({n_singletons / n_s1 * 100:.1f}%)")
    print(f"Total candidate pairs: {total_cands} (avg {total_cands / n_s1:.2f} per entity)")
...
def main():
    ...
    run_prediction(test_dir=args.test_dir, output_dir=args.output_dir)
